...
const MenuGroup = ({menuGroup})=>{
    ...
    return (
        <>
            <ListItemButton onClick={handleClick} sx={{color:'#fff', backgroundColor:'rgb(28, 37, 54)'}}>
                <ListItemIcon sx={{color:'rgb(255, 255, 255)'}}>
                    {menuGroup.icon}
                </ListItemIcon>
                <ListItemText primaryTypographyProps={{fontSize:"16px"}} primary={menuGroup.title} />
                {menuGroup.children ? open ? <ExpandLess/> : <ExpandMore/> : <></>}
            </ListItemButton>
            <Collapse in={open} timeout="auto" unmountOnExit>
                <List component="div" disablePadding sx={{backgroundColor:"rgb(28, 37, 54)"}}>
                    {menuItems}
                </List>
            </Collapse>
        </>
