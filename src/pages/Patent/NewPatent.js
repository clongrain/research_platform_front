...
import supabase, { PATENT_STORAGE_PATH } from "@/config";
...
export default function NewPatent() {
  const [patent, setPatent] = useState({ publication_date: dayjs('') })
  ...
  const checkPatent = () => {
    if (patent.title === undefined || patent.title.trim().split(" ").length === 0) {
      messageAPI.error({ title: 'Title Error', text: '无效的论文题目' })
      return false
    }
    if (patent.author === undefined || patent.author.trim().split(" ").length === 0 ||
      !(/^[a-zA-Z\u4e00-\u9fa5]+$/.test(patent.author.replace(/,/g, '').replace(/ /g, '')))) {
      messageAPI.error({ title: 'Author Name Error', text: '无效的作者名称' })
      return false
    }
    if (patent.conference === undefined || patent.conference.trim().split(" ").length === 0) {
      messageAPI.error({ title: 'Conference Error', text: '无效的会议名称' })
      return false
    }
    if (patent.subject === undefined || patent.subject.trim().split(" ").length === 0) {
      messageAPI.error({ title: 'Subject Error', text: '无效的领域名称' })
      return false
    }
    if (patent.publication_date === undefined || !patent.publication_date) {
      messageAPI.error({ title: 'Publication Date Error', text: '无效的发表日期' })
      return
    }
    if (patent.keyword === undefined || patent.keyword.trim().split(" ").length === 0) {
      messageAPI.error({ title: 'Keyword Error', text: '无效的关键字' })
      return false
    }
    if (patent.doi === undefined || patent.doi.trim().split(" ").length === 0) {
  ...
  return (
    <Container maxWidth=''>
      <Stack sx={{
        ...
        <Box>
          <Link to={'/achievements/patents'} style={{
            display: 'flex',
            alignItems: 'center',
            fontSize: '16px',
            gap: '8px',
          ...
          <Stack sx={{
            ...
            <Grid container columnSpacing={3} rowSpacing={3}>
              <Grid item xs={12} md={5}>
                <Stack sx={{
                  ...
                  <FormControl fullWidth variant="standard">
                    ...
                    <BootstrapInputBase id="bootstrap1" onChange={(e) => setPatent({ ...patent, title: e.target.value })} />
                  </FormControl>
                  <FormControl fullWidth variant="standard">
                    <InputLabel sx={{
                      fontWeight: 500,
                    ...
                    <BootstrapInputBase id="bootstrap1" onChange={(e) => { setPatent({ ...patent, author: e.target.value }) }} />
                  </FormControl>
                  ...
                  <FormControl fullWidth variant="standard">
                    ...
                    }} htmlFor="bootstrap1" shrink>Conference</InputLabel>
                    <BootstrapInputBase id="bootstrap1" onChange={(e) => setPatent({ ...patent, conference: e.target.value })} />
                  </FormControl>
                  <FormControl fullWidth variant="standard">
                    <InputLabel sx={{
                      fontWeight: 500,
                    ...
                    <BootstrapInputBase id="bootstrap1" onChange={(e) => setPatent({ ...patent, subject: e.target.value })} />
                  </FormControl>
                  <StyledDatePicker label='Publication Date' format="YYYY-MM-DD" onChange={(newValue) => { setPatent({ ...patent, publication_date: newValue }) }} />
                  <FormControl fullWidth variant="standard">
                    <InputLabel sx={{
                      fontWeight: 500,
                      fontSize: '1.3rem',
                    ...
                    <BootstrapInputBase id="bootstrap1" onChange={(e) => setPatent({ ...patent, keyword: e.target.value })} />
                  </FormControl>
                  <FormControl fullWidth variant="standard">
                    <InputLabel sx={{
                      fontWeight: 500,
                    ...
                    <BootstrapInputBase id="bootstrap1" onChange={(e) => setPatent({ ...patent, doi: e.target.value })} />
                  </FormControl>
                  ...
                  <FormControl fullWidth variant="standard">
                    ...
                    <BootstrapInputBase multiline maxRows={100} id="bootstrap1" onChange={(e) => setPatent({ ...patent, abstract: e.target.value })} />
                  </FormControl>
                  <FormControl fullWidth variant="standard">
                    <InputLabel sx={{
                      fontWeight: 500,
              ...
              <Grid item xs={12} md={7}>
                <FileUploader uploadPath={PATENT_STORAGE_PATH} object={patent} setObject={setPatent} checkObject={checkPatent} tableName="patent" joinTableName="user_patent" 
