...
export default function PatentDetail() {
  ...
  const fetchData = async () => {
    const { data, error } = await supabase.from('patent').select('*,user(user_id,user_type),project(*)').eq('id', patentId)
  ...
  const handleAddAuthor = async () => {
    if (canEdit) {
      if (mutipleSelectValue.length === 0) {
        ...
        const {data, err} = await supabase.from('user_patent').insert(mutipleSelectValue.map((item) => {
          return { patent_id: patentId, user_id: item.user_id }
        }))
        if(err){
  ...
  const handleClickSaveChanges = async () => {
    const { error } = await supabase.from('patent').update(editPatent).eq('id', patentId)
    if (error) messageAPI.error({ title: 'Failed to update patent', text: error.message })
    else {
      messageAPI.success({ text: 'Successfully update' })
      setPatent(editPatent)
    }
  }
  ...
  return (
    <Container maxWidth=''>
      <Stack sx={{
        ...
        <ReturnLink to={'/achievements/patents'} title={'Patents'} />
        <Stack sx={{
          gap: '32px'
        }}>
          <Stack sx={{
            ...
            {canEdit &&
              <>
                ...
                <RemoveDialog open={dialogOpen} setOpen={setDialogOpen} text={'您确定撤回该成果吗'}
                  invokeMethod={async () => {
                    const { error } = await supabase.from('patent').delete().eq('id', patentId)
          ...
          <Stack gap={'16px'}>
            <StyledTabs variant="scrollable" scrollButtons='auto'
              ...
              <StyledTab icon={<PersonOutlinedIcon />} iconPosition="start" label='Patent Members' />
              ...
              <ContentPaper sx={{
                ...
                <Grid container rowSpacing={3} columnSpacing={3}>
                  <Grid item xs={12} md={6}>
                    <StyledEditTextField fullWidth disabled={!(canEdit && infomarionEdit)} label='Title' value={editPatent.title || ''}
                      onChange={(e) => setEditPatent({ ...editPatent, title: e.target.value })} />
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <StyledEditTextField fullWidth disabled={!(canEdit && infomarionEdit)} label='Inventor' value={editPatent.inventor || ''}
                      onChange={(e) => setEditPatent({ ...editPatent, inventor: e.target.value })} />
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <StyledEditTextField fullWidth disabled={!(canEdit && infomarionEdit)} label='Applicant' value={editPatent.applicant || ''}
                      onChange={(e) => setEditPatent({ ...editPatent, applicant: e.target.value })} />
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <StyledEditTextField fullWidth disabled={!(canEdit && infomarionEdit)} label='Right Holder' value={editPatent.right_holder || ''}
                      onChange={(e) => setEditPatent({ ...editPatent, right_holder: e.target.value })} />
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <StyledEditTextField fullWidth disabled={!(canEdit && infomarionEdit)} label='Application Date' value={editPatent.application_date || ''}
                      onChange={(e) => setEditPatent({ ...editPatent, application_date: e.target.value })} />
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <StyledEditTextField fullWidth disabled={!(canEdit && infomarionEdit)} label='Authorzation Date' value={editPatent.authorzation_date || ''}
                      onChange={(e) => setEditPatent({ ...editPatent, authorzation_date: e.target.value })} />
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <StyledEditTextField fullWidth disabled={!(canEdit && infomarionEdit)} label='Domain' value={editPatent.domain || ''}
                      onChange={(e) => setEditPatent({ ...editPatent, domain: e.target.value })} />
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <StyledEditTextField fullWidth disabled={!(canEdit && infomarionEdit)} label='Patent Number' value={editPatent.patent_number || ''}
                      onChange={(e) => setEditPatent({ ...editPatent, patent_number: e.target.value })} />
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <StyledEditTextField fullWidth disabled={!(canEdit && infomarionEdit)} label='Legal Status' value={editPatent.legal_status || ''}
                      onChange={(e) => setEditPatent({ ...editPatent, legal_status: e.target.value })} />
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <StyledEditTextField fullWidth disabled={!(canEdit && infomarionEdit)} label='Abstract' value={editPatent.abstract || ''}
      ...
      <RemoveDialog open={removeAuthorDialogOpen} setOpen={setRemoveAuthorDialogOpen} text={"您确定将 <" + deleteAuthor.name + "> 从成果作者中删除吗？"} invokeMethod={async () => {
        const { data, error } = await supabase.from('user_patent').delete().eq("user_id", deleteAuthor.id).eq("patent_id", patentId)
        if (error) {
