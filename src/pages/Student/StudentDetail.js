...
export default function StudentDetail() {
  ...
  const fetchData = async () => {
    ...
    const userData = await supabase.from('user_student').select().eq('user_id', id)
  ...
  const handleClickSaveChanges = async () => {
    if (canUpdate && editing) {
      setCanUpdate(false)
      const { data, error } = await supabase.from('user_student').update(editUser).eq('user_id', editUser.user_id).select()
