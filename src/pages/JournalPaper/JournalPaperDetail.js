...
export default function JournalPaperDetail() {
  ...
  const fetchData = async () => {
    const { data, error } = await supabase.from('journal_paper').select('*,user(user_id,user_type),project(*)').eq('id', paperId)
  ...
  const handleAddAuthor = async () => {
    if (canEdit) {
      if (mutipleSelectValue.length === 0) {
        ...
        const { data, err } = await supabase.from('user_jounal_paper').insert(mutipleSelectValue.map((item) => {
  ...
  const handleClickSaveChanges = async () => {
    ...
    const { data, error } = await supabase.from('journal_paper').update(paper).eq('id', paperId)
    if (error) {
      messageAPI.error({ title: 'Update journal paper failed', text: error.message })
    }
    else {
  ...
  return (
    <Container maxWidth='false'>
      <Stack sx={{
        ...
        <ReturnLink to={'/achievements/journal-papers'} title={'Journal Papers'} />
        <Stack sx={{
          gap: '32px'
        }}>
          <Stack sx={{
            ...
            {canEdit &&
              <>
                ...
                <RemoveDialog open={dialogOpen} setOpen={setDialogOpen} text={'您确定撤回该成果吗'}
                  invokeMethod={async () => {
                    const { error } = await supabase.from('journal_paper').delete().eq('id', paperId)
            ...
            {tabValue == 0 &&
              <ContentPaper sx={{
                ...
                <Grid container rowSpacing={3} columnSpacing={3}>
                  ...
                  <Grid item xs={12} md={6}>
                    <StyledEditTextField fullWidth disabled={!(canEdit && infomarionEdit)} label='Journal' value={journal}
      ...
      <RemoveDialog open={removeAuthorDialogOpen} setOpen={setRemoveAuthorDialogOpen} text={"您确定将 <" + deleteAuthor.name + "> 从成果作者中删除吗？"} invokeMethod={async () => {
        const { data, error } = await supabase.from('user_journal_paper').delete().eq("user_id", deleteAuthor.id).eq("paper_id", paperId)
        if (error) {
