...
import { BUCKET_NAME, PAPER_STORAGE_PATH, PAPER_TYPE } from "@/config";
import DownloadOutlinedIcon from '@mui/icons-material/DownloadOutlined';
import { exportJournalPapers } from "@/action/ExportAction";
...
export default function JournalContent() {
  ...
  const handleCilckDetail = (data) => {
    navigate(`/achievements/journal-papers/details/${data.id}`)
  }
  ...
  const handleExport = () => {
    ...
    if(!selectedData) messageAPI.info({text:'Please select at least one piece of data'})
      ...
      dispatch(exportJournalPapers(datas.filter((data,index)=>selected[index])))
    }
  }
  return (
    <Container maxWidth=''>
      <Stack gap={'32px'}>
        <Stack sx={{
          ...
          <Stack justifyContent={'center'}>
            <ContentButton startIcon={<AddIcon />} onClick={() => { navigate("/achievements/journal-papers/new") }}>
              Add
            </ContentButton>
          </Stack>
        </Stack>
        <ContentPaper>
          <Tabs value={tabValue} onChange={handleChangeTab} aria-label="tabs" sx={{ pr: '24px', pl: '24px' }}>
            ...
            <Tab sx={{ textTransform: 'none', fontFamily: fontFamily }} label="CCF B" />
            <Tab sx={{ textTransform: 'none', fontFamily: fontFamily }} label="CCF C" />
          </Tabs>
          ...
          <Stack sx={{
            ...
            <Box sx={{
              ...
              <SearchTextField searchValue={searchValue} setSearchValue={setSearchValue} handleSearchEnter={handleSearchEnter} placeholder={"Search journal papers"} />
            </Box>
