...
import { exportAwards } from "@/action/ExportAction";
...
export default function AwardContent() {
  const columns = [
    { show:'名称',field:"title" },
    { show:'获奖时间',field:"presentation_date" },
    { show:'颁奖单位',field:"awarding_unit" },
    { show:'获奖人',field:"prizewinner" }
  ]
  ...
  const handleExport = () => {
    ...
    if (!selectedData) messageAPI.info({ text: 'Please select at least one piece of data' })
      ...
      dispatch(exportAwards(datas.filter((data, index) => selected[index])))
    }
  }
  return (
      ...
      <Stack gap={'32px'}>
        <Stack sx={{
          ...
          <Stack justifyContent={'center'}>
            <ContentButton startIcon={<AddIcon />} onClick={() => { navigate("/achievements/awards/new") }}>
              Add
            </ContentButton>
          </Stack>
        </Stack>
        <ContentPaper>
          ...
          <Stack sx={{
            ...
            <Box sx={{
              ...
              <SearchTextField searchValue={searchValue} setSearchValue={setSearchValue} handleSearchEnter={handleSearchEnter} placeholder={"Search awards"} />
            </Box>
                            ...
                            <ContentTableBodyCell>
                              <Stack>
                                ...
                                <Typography sx={{
                                  ...
                                  fontSize: '0.85rem',
                                }}>{data.level}</Typography>
                              </Stack>
                            </ContentTableBodyCell>
                            <ContentTableBodyCell>
                              <AvatarGroup max={3} sx={{ justifyContent: 'flex-end' }}>
                                {
                                  data.prizewinner !== undefined && data.prizewinner && data.prizewinner.split(',').map((item, index) => {
                                    return (
