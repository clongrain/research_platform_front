...
import { Stack, styled, Container, Paper, Tabs, Tab, Box, TextField, Typography, Grid, Button, Table, TableHead, TableRow, TableBody, Avatar, Autocomplete, IconButton, Tooltip } from "@mui/material";
...
export default function AwardDetail() {
  ...
  const fetchData = async () => {
    const { data, error } = await supabase.from('award').select('*,user(user_id,user_type),project(*)').eq('id', awardId)
  ...
  const handleAddAuthor = async () => {
    if (canEdit) {
      if (mutipleSelectValue.length === 0) {
        ...
        const { data, err } = await supabase.from('user_award').insert(mutipleSelectValue.map((item) => {
          return { award_id: award.id, user_id: item.user_id }
        }))
        if (err) {
  ...
  const handleClickSaveChanges = async () => {
    const { error } = await supabase.from('award').update(editAward).eq('id', awardId)
    if (error) messageAPI.error({ title: 'Failed to update award', text: error.message })
    else {
  ...
  return (
    <Container maxWidth=''>
      <Stack sx={{
        ...
        <ReturnLink to={'/achievements/awards'} title={'Awards'} />
        <Stack sx={{
          gap: '32px'
        }}>
          <Stack sx={{
          ...
          <Stack gap={'16px'}>
            <StyledTabs variant="scrollable" scrollButtons='auto'
              ...
              <StyledTab icon={<PersonOutlinedIcon />} iconPosition="start" label='Award Winners' />
              ...
              <ContentPaper sx={{
                ...
                <Grid container rowSpacing={3} columnSpacing={3}>
                  <Grid item xs={12} md={6}>
                    <StyledEditTextField fullWidth disabled={!(canEdit && infomarionEdit)} label='Title' value={editAward.title || ''}
                      onChange={(e) => setEditAward({ ...editAward, title: e.target.value })} />
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <StyledEditTextField fullWidth disabled={!(canEdit && infomarionEdit)} label='Level' value={editAward.level || ''}
                      onChange={(e) => setEditAward({ ...editAward, level: e.target.value })} />
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <StyledEditTextField fullWidth disabled={!(canEdit && infomarionEdit)} label='Prize Winner' value={editAward.prizewinner || ''}
                      onChange={(e) => setEditAward({ ...editAward, prizewinner: e.target.value })} />
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <StyledEditTextField fullWidth disabled={!(canEdit && infomarionEdit)} label='Awarding Unit' value={editAward.awarding_unit || ''}
                      onChange={(e) => setEditAward({ ...editAward, awarding_unit: e.target.value })} />
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <StyledEditTextField fullWidth disabled={!(canEdit && infomarionEdit)} label='Presentation Date' value={editAward.presentation_date || ''}
                      onChange={(e) => setEditAward({ ...editAward, presentation_date: e.target.value })} />
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <StyledEditTextField fullWidth disabled={!(canEdit && infomarionEdit)} label='Reason' value={editAward.reason || ''}
                      onChange={(e) => setEditAward({ ...editAward, reason: e.target.value })} />
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <Box sx={{
                      width: '100%',
                      ...
                      <Stack sx={{ width: '100%' }} gap={'16px'}>
                        <Button disabled={!canEdit} onClick={() => canEdit && setInfomationEdit(!infomarionEdit)}
                          sx={{
                            color: 'rgb(33, 43, 54)',
                            backgroundColor: '#fff',
                            textTransform: 'none',
                ...
                <Stack sx={{
                  ...
                  <Grid container columnSpacing={3} rowSpacing={3} sx={{
                    ...
                    <Grid item xs={12} sm={7} md={6}>
                      <Autocomplete disableCloseOnSelect fullWidth multiple
                        options={mutipleSelectDatas} getOptionLabel={(option) => option.name}
                    ...
                    <Grid item xs={12} sm={5} md={3}>
                      <TooltipButton disabled={!canEdit} title={"No Permission"} children={
      ...
      <RemoveDialog open={removeAuthorDialogOpen} setOpen={setRemoveAuthorDialogOpen} text={"您确定将 <" + deleteAuthor.name + "> 从成果作者中删除吗？"} invokeMethod={async () => {
        const { data, error } = await supabase.from('user_award').delete().eq("user_id", deleteAuthor.id).eq("award_id", awardId)
        if (error) {
