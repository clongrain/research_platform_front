...
import { exportMonographs } from "@/action/ExportAction";
...
export default function MonographContent() {
  const columns = [
    { show:'名称',field:"title" },
    { show:'作者',field:"author" },
    { show:'出版社',field:"publishers" },
    { show:'发表日期',field:"publication_date" },
    { show:'关键词',field:"keyword" },
    { show:'ISBN',field:"isbn" }
  ]
  ...
  const handleExport = () => {
    ...
    if (!selectedData) messageAPI.info({ text: 'Please select at least one piece of data' })
      ...
      dispatch(exportMonographs(datas.filter((data, index) => selected[index])))
    }
  }
  return (
    <Container maxWidth=''>
      <Stack gap={'32px'}>
        <Stack sx={{
          ...
          <Stack justifyContent={'center'}>
            <ContentButton startIcon={<AddIcon />} onClick={() => { navigate("/achievements/monographs/new") }}>
              Add
            </ContentButton>
          </Stack>
        </Stack>
        <ContentPaper>
          ...
          <Stack sx={{
            ...
            <Box sx={{
              ...
              <SearchTextField searchValue={searchValue} setSearchValue={setSearchValue} handleSearchEnter={handleSearchEnter} placeholder={"Search Monographs"} />
            </Box>
