...
export default function MonographDetail() {
  ...
  const monographId = useParams()?.id
  //渲染信息的字段顺序
  const keys = []
  ...
  const [editMonograph, setEditMonograph] = useState({})
  ...
  const fetchData = async () => {
    const { data, error } = await supabase.from('monograph').select('*,user(user_id,user_type),project(*)').eq('id', monographId)
  ...
  const handleAddAuthor = async () => {
    if (canEdit) {
      if (mutipleSelectValue.length === 0) {
        ...
        const { data, err } = await supabase.from('user_monograph').insert(mutipleSelectValue.map((item) => {
          return { monograph_id: monographId, user_id: item.user_id }
        }))
        if (err) {
  ...
  const handleClickSaveChanges = async () => {
    const { error } = await supabase.from('monograph').update(editMonograph).eq('id', monographId)
    if (error) messageAPI.error({ title: 'Failed to update monograph', text: error.message })
    else {
      messageAPI.success({ text: 'Successfully update' })
      setMonograph(editMonograph)
    }
  }
  ...
  return (
    <Container maxWidth='false'>
      <Stack sx={{
        ...
        <ReturnLink to={'/achievements/monographs'} title={'Monographs'} />
        <Stack sx={{
          gap: '32px'
        }}>
          <Stack sx={{
            ...
            {canEdit &&
              <>
                ...
                <RemoveDialog open={dialogOpen} setOpen={setDialogOpen} text={'您确定撤回该成果吗'}
                  invokeMethod={async () => {
                    const { error } = await supabase.from('monograph').delete().eq('id', monographId)
          ...
          <Stack gap={'16px'}>
            <StyledTabs variant="scrollable" scrollButtons='auto'
              ...
              <StyledTab icon={<PersonOutlinedIcon />} iconPosition="start" label='Monograph Author' />
              ...
              <ContentPaper sx={{
                ...
                <Grid container rowSpacing={3} columnSpacing={3}>
                  <Grid item xs={12} md={6}>
                    <StyledEditTextField fullWidth disabled={!(canEdit && infomarionEdit)} label='Title' value={editMonograph.title || ''}
                      onChange={(e) => setEditMonograph({ ...editMonograph, title: e.target.value })} />
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <StyledEditTextField fullWidth disabled={!(canEdit && infomarionEdit)} label='Author' value={editMonograph.author || ''}
                      onChange={(e) => setEditMonograph({ ...editMonograph, author: e.target.value })} />
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <StyledEditTextField fullWidth disabled={!(canEdit && infomarionEdit)} label='Publication Date' value={editMonograph.publication_date || ''}
                      onChange={(e) => setEditMonograph({ ...editMonograph, publication_date: e.target.value })} />
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <StyledEditTextField fullWidth disabled={!(canEdit && infomarionEdit)} label='Publisher' value={editMonograph.publisher || ''}
                      onChange={(e) => setEditMonograph({ ...editMonograph, publisher: e.target.value })} />
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <StyledEditTextField fullWidth disabled={!(canEdit && infomarionEdit)} label='Keyword' value={editMonograph.keyword || ''}
                      onChange={(e) => setEditMonograph({ ...editMonograph, keyword: e.target.value })} />
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <StyledEditTextField fullWidth disabled={!(canEdit && infomarionEdit)} label='ISBN' value={editMonograph.isbn || ''}
                      onChange={(e) => setEditMonograph({ ...editMonograph, isbn: e.target.value })} />
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <StyledEditTextField fullWidth disabled={!(canEdit && infomarionEdit)} label='Language' value={editMonograph.language || ''}
                      onChange={(e) => setEditMonograph({ ...editMonograph, language: e.target.value })} />
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <StyledEditTextField fullWidth disabled={!(canEdit && infomarionEdit)} label='Abstract' value={editMonograph.abstract || ''}
                      onChange={(e) => setEditMonograph({ ...editMonograph, abstract: e.target.value })} />
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <Box sx={{
                      width: '100%',
      ...
      <RemoveDialog open={removeAuthorDialogOpen} setOpen={setRemoveAuthorDialogOpen} text={"您确定将 <" + deleteAuthor.name + "> 从成果作者中删除吗？"} invokeMethod={async () => {
        const { data, error } = await supabase.from('user_monograph').delete().eq("user_id", deleteAuthor.id).eq("monograph_id", monographId)
        if (error) {
